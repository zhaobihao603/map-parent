...
import com.imap.cloud.common.dto.NewstuguiderouteDto;
/**
 * 新生指引实体类
 * @author 99901786
 *
 */
public class Newstuguideroute {
    ...
    private List<TemporaryNodeDto> listTemporaryNode;	//临时点集合
    
	public List<TemporaryNodeDto> getListTemporaryNode() {
		return listTemporaryNode;
	}
	public void setListTemporaryNode(List<TemporaryNodeDto> listTemporaryNode) {
		this.listTemporaryNode = listTemporaryNode;
	}
	public String getId() {
		return id;
	}
	...
	public NewstuguiderouteDto toNewstuguiderouteDTO(){
		NewstuguiderouteDto date=new NewstuguiderouteDto();
		date.setId(id);
		date.setArea(area);
		date.setAudited(audited);
		date.setBusLine(busLine);
		date.setCollege_id(college_id);
		date.setDatum(datum);
		date.setFabu(fabu);
		date.setFabuState(fabuState);
		date.setGuide_date(guide_date);
		date.setGuideline(guideline);
		date.setList(list);
		date.setName(name);
		date.setYktId(yktId);
		date.setTrafficBusId(trafficBusId);
		date.setReportingTime(reportingTime);
		date.setPublicServiceId(publicServiceId);
		return date;
	}
